...
async def process_selected_month_for_add_salary(callback_query: CallbackQuery, state):
    try:
        ...
        print(f"Ошибка при добавлении зарплаты в выбранный месяц: {e}")
        await callback_query.message.answer("Произошла ошибка при добавлении зарплаты.")
        await callback_query.answer()
...
def register_salary(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(process_selected_month_for_add_salary, text_startswith="month_", state=SalaryForm.waiting_for_month)
