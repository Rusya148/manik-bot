...
from app.bot.keyboards import kb_back_inline
from app.bot.states import DeleteForm, Form
    ...
    if current_state == Form.waiting_for_name.state:
        ...
        if current_state == Form.waiting_for_link.state:
            ...
            await callback_query.message.edit_text(
                "Введите время, на которое записан клиент: ", reply_markup=kb_back_inline
            )
            await Form.waiting_for_time.set()
        elif current_state == Form.waiting_for_prepayment.state:
            await callback_query.message.edit_text(
                "Введите дату записи клиента (формат: DD.MM.YYYY): ", reply_markup=kb_back_inline
            )
